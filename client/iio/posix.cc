...
void EventIIO::initialize_connection(){
        if (_cert == "" || _key == "") {
                char buf[1024];
...
EventIIO::EventIIO(std::string remoteHost, uint16_t port, std::string&& certificate, std::string&& key, std::string&& trusted): _remoteHost(remoteHost), _port(port), _cert(certificate), _key(key), _trusted(trusted), _fd(-1), _ssl(NULL), _connected(false){
...
std::shared_ptr<IBufferWrapper> EventIIO::Send(std::shared_ptr<IBufferWrapper> buf){
